...
- **`provider/` + `engine/`** — the CrewHQ-hosted Claim Laundering workspace. It mirrors the provider-facing RCM pages while the CEO/Operations RCM portal remains separately gated. `crew-provider-gate.js` reuses the CrewOS PIN session, and `rcm-cloud-token.js` exchanges that signed session for a five-minute Google Cloud token. Keep these shared provider/engine files synchronized with `murraydotcom/bhw-rcm-platform` until CrewHQ intentionally changes its tile to the final RCM provider link.
...
- **Notifications — Keragon replacement** (inbound comms → Patient Request Triage Queue → Front Desk OS inbox, bucketed by `Source`): **`lib/triage.js`** (shared patient-match-by-phone + queue writer + Dialpad JWT verify), **`dialpad-events.js`** (inbound SMS + missed calls + voicemails), **`ifax-events.js`** (inbound fax webhook), **`email-ingest.js`** (parses iFax/Dialpad *emails* forwarded by `docs/gmail-ingest.gs`, since consumer iFax delivers by email not webhook). Answered/outbound events are ignored. Every inbound webhook **fails closed** — it requires its own shared secret and returns `503` until that secret is set (`DIALPAD_WEBHOOK_SECRET` — signed-JWT bodies only, raw JSON rejected; `IFAX_WEBHOOK_SECRET`; `EMAIL_INGEST_SECRET`; `CRISP_INGEST_TOKEN` for `crisp.js` ADT ingest), so none is ever an open write into the triage queue.
...
- **Environment variables** (Netlify): core is `NOTION_TOKEN`, `SESSION_SECRET`, `SETUP_SECRET`. Individual functions also use `RESEND_API_KEY`/`RESEND_FROM` (email), `DASH_KEY`, `MASTER_DB_ID`/`QUEUE_DB_ID`/`PATIENTS_DB_ID`/`EVENTS_DB_ID`, `DIALPAD_TOKEN`/`DIALPAD_WEBHOOK_SECRET`/`DIALPAD_FROM`, `STEDI_KEY_PREFIX`/`STEDI_KEY_SUFFIX`, `OPENAI_API_KEY`/`OPENAI_MODEL` (Paperwork AI prefill), `IFAX_WEBHOOK_SECRET` (required to arm the iFax webhook) and `EMAIL_INGEST_SECRET` (required to arm `email-ingest`; must match the secret in the Gmail Apps Script). Inbound webhooks fail closed: `dialpad-events`, `ifax-events`, `email-ingest`, and the `crisp` ADT POST all `503` until their secret (`DIALPAD_WEBHOOK_SECRET` / `IFAX_WEBHOOK_SECRET` / `EMAIL_INGEST_SECRET` / `CRISP_INGEST_TOKEN`) is set.
- **Claim Laundering cloud access:** set `RCM_CLOUD_API_URL` to the deployed Cloud Run API and `CREWHQ_CLOUD_TOKEN_SECRET` to the same dedicated secret configured on that Cloud Run service. The token exchange fails closed when either is absent.
- **Shared operations cloud access:** `BHWcrewOS-latest/cloud/operations-api/` owns Google-native `patientRequests`, tasks, communication records, and metadata-only audit events. Set `OPERATIONS_CLOUD_API_URL` plus `CREWOS_OPERATIONS_TOKEN_SECRET` for staff access; the secure Care Connect intake bridge separately uses `CARE_CONNECT_INTAKE_SECRET`. It deliberately does not send SMS/Google Chat, does not replace the medication page/service, treats RCM as downstream, and never calls WelcomeToBHW.
...
**Critical gotcha:** every `package.json` (root, `netlify/functions/`, and the Cloud Run service) must be valid JSON. A malformed one (e.g. a missing closing brace) fails the deploy before any content is published — the error looks unrelated to the actual edit.

## Working in this repo

- Focused backend checks use Node's built-in test runner (`npm test` from `BHWcrewOS-latest/`). Static UI changes still need a Netlify deploy preview or `netlify dev`.
