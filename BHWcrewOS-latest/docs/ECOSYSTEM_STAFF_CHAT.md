...
- CrewOS popup sign-in on the first visit to each staff site and whenever its signed-in account changes. Subsequent pages on that site reuse the same chat identity. Tokens stay within the CrewHQ origin; parents receive only open/close/visibility and badge-count messages. An opaque host-account fingerprint can only clear a stale chat session, never authenticate it. No tokens, names, message bodies or clinical context cross the frame boundary. Host sign-out clears chat; an account change cannot inherit another staff member's conversation history.
