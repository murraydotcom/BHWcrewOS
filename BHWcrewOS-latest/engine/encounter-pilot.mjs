  ...
  try {
    ...
    return parsed.encounters.map((metadata) => {
      ...
      return buildEncounterPacket({
        ...
        orders: Array.isArray(sessionClinical?.orders) ? sessionClinical.orders : [],
        referrals: Array.isArray(sessionClinical?.referrals) ? sessionClinical.referrals : [],
        followUp: Array.isArray(sessionClinical?.followUp) ? sessionClinical.followUp : [],
        patientInstructions: Array.isArray(sessionClinical?.patientInstructions) ? sessionClinical.patientInstructions : [],
        pendingResults: Array.isArray(sessionClinical?.pendingResults) ? sessionClinical.pendingResults : [],
        returnPrecautions: Array.isArray(sessionClinical?.returnPrecautions) ? sessionClinical.returnPrecautions : [],
...
export function buildCharmPacket(encounter = {}, now = new Date()) {
  ...
  return {
    ...
    packet: {
      ...
      medications: [].concat(encounter.medications || []),
      orders: [].concat(encounter.orders || []),
      referrals: [].concat(encounter.referrals || []),
      followUp: [].concat(encounter.followUp || []),
      patientInstructions: [].concat(encounter.patientInstructions || []),
      pendingResults: [].concat(encounter.pendingResults || []),
      returnPrecautions: [].concat(encounter.returnPrecautions || []),
    },
  };
}
