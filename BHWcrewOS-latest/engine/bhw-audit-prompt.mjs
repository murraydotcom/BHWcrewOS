...
- Condition/topic: ... | Source: [current primary source] | Year: [publication/update year] | Note: ...
...
- Format each finding exactly like this: 1. [HIGH] Issue: ... | Location: ... | Suggested fix: ... | Supporting source: [current primary source + year, or BHW policy + year]
...
export function buildBhwChartAuditPrompt(encounter = {}, { auditedOn = new Date().toISOString().slice(0, 10) } = {}) {
  ...
  const structured = {
    medications: [].concat(encounter.medications || []).map((item) => typeof item === "string" ? item : {
      name: item?.name || "",
