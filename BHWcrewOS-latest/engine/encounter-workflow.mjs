...
export function detectOutputs(noteText = "", structuredInput = {}) {
  ...
  if (structured.orders.length || /lab|cbc|cmp|a1c|imaging|x-ray|mri|ct scan|ultrasound/i.test(note)) add("order", "Orders summary", "Laboratory or diagnostic work was discussed.");
  if (structured.followUp.length || /follow[- ]?up|return (in|to)|rtc/i.test(note)) add("follow_up", "Follow-up task", "A follow-up interval was documented.");
  if (structured.medications.length || /start|increase|decrease|discontinue|continue .*mg|prescrib/i.test(note)) add("medication", "Medication-change verification", "Medication-management language was detected.");
  add("instructions", "After-visit summary (AVS)", "Create the patient-facing AVS from the structured encounter packet for every completed encounter.");
  return items;
}
...
export function buildEncounterPacket(input = {}) {
  ...
  const structured = normalizeStructuredEncounter(input, note);
  const packet = {
