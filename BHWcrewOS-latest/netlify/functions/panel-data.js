...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (action === "searchRegistry") {
      ...
      if (query.length < 2) return json(200, { patients: [] });
      const patients = searchCloudPatients(await listCloudPatients(session), query, 20)
        .filter((patient) => patient.selectable !== false)
