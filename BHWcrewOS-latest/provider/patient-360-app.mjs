import { leaveTranscript } from "../assets/leave-schema.mjs";
...
let leaveConnection={requests:[],error:""};
...
function serviceRequestDetail(item) {
  const requestId = (item.identifier || []).find((identifier) => String(identifier.system || "").endsWith("/patient-request"))?.value || item.id || "";
  const workflowStatus = (item.extension || []).find((extension) => String(extension.url || "").endsWith("/bhw-referral-workflow-status"))?.valueCode || "";
  return compact([
    item.resourceType,
    item.status,
    item.intent,
    requestId ? `Request ${requestId}` : "",
    workflowStatus ? `CrewOS ${statusText(workflowStatus)}` : "",
  ]).join(" - ");
}
  ...
  $("content").innerHTML = `
    ...
    <section class="worksheet" id="data"><div class="worksheet-heading"><span class="section-number gold">6</span><h2>Digital Clinical Data Spine</h2><p>Everything the paper worksheet cannot continuously hold</p></div>
      <div class="data-grid">
        ...
        ${dataCard("Orders, referrals & tasks", [...serviceRequests, ...tasks], (item) => renderResourceItem(item, item.resourceType === "ServiceRequest" ? serviceRequestDetail(item) : compact([item.resourceType, item.status, item.intent]).join(" - ")), "No structured order, referral or task is connected.")}
...
const PAGE_LINKS = [
  ...
  ["nutrition", "Nutrition", "nutrition-intelligence.html"],
...
function leaveIntakePanel() {
 return `<section class="panel"><div class="panel-head"><h3>Leave paperwork — patient-reported</h3><a class="btn" href="/bhw-paperwork.html?patient=${encodeURIComponent(PATIENT_ID)}">Open Paperwork Studio</a></div><p>Intake evidence for clinical review; does not change diagnoses or the approved care plan.</p>${leaveConnection.error?`<p>${esc(leaveConnection.error)}</p>`:leaveConnection.requests.length?leaveConnection.requests.map(item=>`<details><summary>${esc(item.formType)} · ${esc(dateText(item.submittedAt,true))} · Awaiting review</summary><pre style="white-space:pre-wrap">${esc(leaveTranscript(item))}</pre></details>`).join(""):"<p>No submitted leave requests.</p>"}</section>`;
}
...
function overviewPage(context) {
  const { patient, conditions, observations, medications, encounters, tasks, carePlans, allergies, reports, goals, serviceRequests, clinicalImpressions, timeline, careTeams, preventiveCare, nutritionIntelligence } = context;
  ...
  const reviewedNutrition = nutritionIntelligence?.patientVisible && nutritionIntelligence?.status === "published-provider-reviewed"
    ? `<ul class="clinical-list"><li><b>${esc(statusText(nutritionIntelligence.energyDirection || "direction not recorded"))}</b><small>Provider-reviewed energy direction</small></li><li><b>${esc(statusText(nutritionIntelligence.dietaryPattern?.selectedBasePattern || "pattern not recorded"))}</b><small>Reviewed dietary pattern; not a weight-management default</small></li><li><b>${esc((nutritionIntelligence.nutritionState || []).map(statusText).join(" · ") || "No phenotype label published")}</b><small>Nutrition phenotypes are decision-support labels, not diagnoses</small></li>${nutritionIntelligence.safetyFlags?.length ? `<li><b>${esc(nutritionIntelligence.safetyFlags.map(statusText).join(" · "))}</b><small>Open safety flags in the published clinical summary</small></li>` : ""}</ul>`
    : `<p class="overview-empty">No separately published provider-reviewed Nutrition Intelligence summary is connected.</p>`;
  return `<section class="worksheet health-overview"><div class="worksheet-heading"><span class="section-number overview">360</span><h2>Whole-Person Health Snapshot</h2><p>What is active, important, due, and shaping this patient's health now</p></div>
    ...
    <div class="overview-clinical-grid"><section class="panel"><div class="panel-head"><h3>Nutrition & individualized plan</h3><a class="btn" href="nutrition-intelligence.html">Nutrition Intelligence</a></div><div class="panel-body personalized-grid"><div><span>Published Nutrition Intelligence</span>${reviewedNutrition}</div><div><span>Existing Blueprint and care-plan context</span>${compactItems(nutrition,(item)=>`<b>${esc(titleText(item))}</b><small>${esc(item.description || item.status || "recorded")}</small>`,"No additional structured nutrition care-plan item is connected.",2)}<ul class="clinical-list"><li><b>${esc(carePlans[0] ? titleText(carePlans[0]) : "Health Blueprint not connected")}</b><small>Nutrition output enters the Blueprint only as review-required source material</small></li></ul></div></div></section><section class="panel"><div class="panel-head"><h3>Prominent timeline</h3><a class="btn" href="patient-360-timeline.html">Full timeline</a></div><div class="panel-body prominent-timeline">${latestTimeline.length ? latestTimeline.map((event)=>`<div><time>${esc(dateText(event.date))}</time><span><b>${esc(event.label || event.type)}</b><small>${esc(event.type || "Clinical event")}</small></span></div>`).join("") : `<p class="overview-empty">No high-value timeline events are connected.</p>`}</div></section></div>
    ${leaveIntakePanel()}${patientCheckinPanel(context)}
...
async function load() {
  ...
  try {
    ...
    try {leaveConnection={requests:await cloudClient.listLeaveRequests(PATIENT_ID),error:""};}
    catch(error){leaveConnection={requests:[],error:"Leave intake could not be loaded from BHW Cloud. Reconnect to review submitted requests."};}
