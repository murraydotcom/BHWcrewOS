...
5. **Synthetic transcription pilot** — a staff role-play recorder that sends
   audio directly to the authenticated Cloud Run endpoint, returns an editable
   speaker-labeled draft, and requires provider review before copy.

The first transcription release is locked to reserved test ID `BHW0000` and
rejects real patient IDs. Audio and transcript text are not written to Firestore
or browser storage. Real-patient use remains disabled until BHW approves the
recording-consent, retention/deletion, access, incident-response, and
legal-record workflow.

The Cloud Run API accepts the restricted RCM portal and the CrewHQ provider
mirror as separate origins. CrewHQ exchanges its signed PIN session for a
five-minute token using a dedicated `CREWHQ_CLOUD_TOKEN_SECRET`; it does not
expose the CEO/Operations RCM session or credentials.
...
provider/transcription.html ← synthetic-only microphone and transcript review pilot.
provider/patient-registry.html ← protected CrewOS patient master list and encounter launch point.
