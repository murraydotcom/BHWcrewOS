...
exports.handler = async (event) => {
  try {
    ...
    if (qs0.dir) {
      // ---- Specialist Directory (for Referral Finder + Generator) ----
      const dres = await fetch(`${NOTION}/databases/${process.env.SPECIALIST_DB_ID}/query`, {
        method: 'POST', headers: H(),
        body: JSON.stringify({ page_size: 100, sorts: [{ property: 'Specialist', direction: 'ascending' }] }),
      });
      const dd = await dres.json();
      const specialists = (dd.results || []).map(r => {
        const P = r.properties || {};
        const text = f => (P[f]?.rich_text?.[0]?.plain_text) || '';
        return {
          name: P['Specialist']?.title?.[0]?.plain_text || '',
          specialty: P['Specialty']?.select?.name || '',
          org: text('Practice/Institution'),
          address: text('Address'),
          phone: P['Phone']?.phone_number || '',
          fax: P['Fax']?.phone_number || '',
          networks: (P['Networks Accepted']?.multi_select || []).map(m => m.name),
          preferred: !!P['⭐ Preferred']?.checkbox,
          wait: P['Typical Wait']?.select?.name || '',
          accepting: !!P['Accepting New Patients']?.checkbox,
        };
      }).filter(s => s.name);
      return { statusCode: 200, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ specialists }) };
    }
