...
function stediRequest(path, body) {
...
async function runCheck(patient) {
  ...
  const res = await stediRequest("/2024-04-01/change/medicalnetwork/eligibility/v3", payload);
  if (!res.ok) {
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (b.action === "check") {
      ...
      const result = await runCheck(patient);
    ...
    if (b.action === "batch") {
      ...
      for (const patient of slice) {
        try { results.push({ name: patient.name, ...(await runCheck(patient)) }); }
